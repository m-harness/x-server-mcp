...
- 環境変数 `XSERVER_SERVER_NAME` が設定されている場合はデフォルト値として使用（実質必須。未設定時は毎回引数で指定が必要）
...
|---------|---------|--------------|---------|------|
| `apikey-info` | GET | `/v1/me` | 読取 | 認証中のAPIキー情報を取得。**注意: APIキー情報がLLMに渡されるため、疎通確認には `server-info` を推奨** |
